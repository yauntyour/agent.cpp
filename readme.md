...
**agent.cpp** 是一个完全透明、高度可控的 C++ 实现的完整 Agent 系统，具备完整的 Agent 工具链支持。与庞大的 Openclaw 系统相比，agent.cpp 仅依赖于几个核心文件组成的通信系统和工具系统，内存占用极低（完全取决于你的上下文有多长，系统开销基本为0），可轻松部署资源受限的环境中，仅需提供 Asio 和 JSON 依赖。
...
可以在配置文件中修改触发自动记忆摘要的上下文的字数，超过时系统会自动生成会话记忆并且更新会话上下文。
