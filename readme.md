...
- **增量记忆系统**：自动/手动将对话历史摘要为记忆，支持增量更新（仅处理新消息），单次 LLM 调用同时生成摘要与关键词，显著降低 Token 消耗。
- **自动触发记忆压缩**：上下文长度超过 `max_context` 阈值时自动触发记忆保存，避免上下文溢出。
- **频道支持**：内置 Telegram 与 WeChat 频道驱动，支持多频道并发接入与独立会话记忆。
- **会话记录热加载**：切换会话或频道时，记忆与聊天记录无缝动态加载，图片数据自动恢复。
- **记忆自动演进**：基于既有记忆与最新交互增量更新记忆内容，实现经验积累的闭环。
